...
if (mapCanvas != undefined) {
	...
	eventListenerInit();
	colorChange(selectedTool);
	var l = cardContainer.childElementCount;
	for(var i = 0; i < l; i++) {
		var canv = cardContainer.childNodes[i].childNodes[1];
		var data = cardContainer.childNodes[i].lastChild;
		cardContainer.childNodes[i].lastChild.pop();
		var t = new displayCanvasState(canv, data);
	}
}